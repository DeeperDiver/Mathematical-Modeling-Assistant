...
% \threelinetable[label]{标题}{列定义}{表头}{表格内容}：三线表，标题在表格上方
% 居中 10.5pt 黑体加粗，自动编号（表 1、表 2…）；label 可选，供正文 \ref。
% 调用示例：\threelinetable[tab:demo]{示例}{ccc}{A & B & C}{1 & 2 & 3 \\ 4 & 5 & 6}
\newcounter{modeltable}
\newcommand{\threelinetable}[5][]{%
  \refstepcounter{modeltable}%
  \ifx\relax#1\relax\else\label{#1}\fi
  \begin{center}
    {\fontsize{10.5pt}{12.6pt}\heiti\bfseries 表~\themodeltable\quad #2}\par
    \vspace{0.6em}
    \begin{tabular}{#3}
      \toprule
      #4 \\
