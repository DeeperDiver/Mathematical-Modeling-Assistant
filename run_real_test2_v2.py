"""端到端运行 real_test2（C 题 NIPT），启用详细日志并捕获所有产物。

用法：
    python run_real_test2_v2.py
"""
...
Path("outputs2_v10").mkdir(parents=True, exist_ok=True)

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(name)s: %(message)s",
    handlers=[
        logging.FileHandler("outputs2_v10/run.log", mode="w", encoding="utf-8"),
...
logger = logging.getLogger("e2e_test2_v2")


def main() -> None:
    problem_text = Path("real_test2/problem.txt").read_text(encoding="utf-8")
    logger.info("读取问题文本: %d 字符", len(problem_text))

    settings = load_settings(".env")
    settings.output_dir = Path("outputs2_v10")
    ...
    attachment_paths = [str(Path("real_test2/附件.xlsx").resolve())]
    ...
    config = {"configurable": {"thread_id": "e2e-real-test2-v2"}}
