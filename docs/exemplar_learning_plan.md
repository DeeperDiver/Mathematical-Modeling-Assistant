...
> 版本：v1.2　日期：2026-08-19　状态：**已实现（P0~P3 完成；P4 已完成全量学习、留一验证与检索验证，端到端开/关对照待完整图流程执行）**
...
| LLM 卡片提炼 | prompts/templates/exemplar_ingest.md | 5 组 (题目, 论文) 产出 5 张合规卡片 | ✅（已全量摄入 2018~2025 国赛优秀论文 93 篇） |
| 题型聚合 | 聚合函数 + 二次提炼 prompt | 产出 ≥2 份题型指南，共性条目有 ≥3 篇来源 | ✅（章节归一化 + 动态阈值，5 份题型指南） |
...
| 任务 | 交付物 | 验收标准 | 状态 |
|---|---|---|---|
| 注入强度/Dropout | exemplar_loader_node + 配置 | 单测验证概率注入 | ✅（强度数值作为注入概率 + writing 额外 dropout） |
...
| 任务 | 交付物 | 验收标准 | 状态 |
|---|---|---|---|
| 真实题对照实验 | 开/关知识库的端到端对比报告 | 用 real_tests 题目跑通，报告结构/图表/文风差异 | ⏳（检索已用三道 real_tests 题验证命中，端到端图对照待跑） |
| 参数调优 | 阈值、强度、dropout 调优记录 | 依据留一评分调整默认值 | ✅（注入多样性按题号去重；相关性阈值改为提示性，题型命中即注入） |
| 文档更新 | README + 本计划标记完成状态 | 运行方式、目录说明完整 | ✅ |

## 八、2026-08-19 全量学习与质量改进记录

- **全量摄入**：2018~2025 国赛优秀论文 93 篇全部提炼为 L1 卡片（含 3 篇解析/评述）；
  67 篇扫描件经 RapidOCR 识别（39 篇前 20 页 + 22 篇前 10 页 + 5 篇文本层补 OCR），
  26 篇文本层 PDF 提取全文缓存，93 篇全部有可检索文本。
- **指南质量**：`optimization` 共性章节从 19 个（含"模型的建立"等命名噪声）精炼为 7 个
  （章节名归一化：去"的"、统一"模型建立/模型求解/模型假设/符号说明"等规范名；
  动态阈值 max(3, 组大小×20%) 防止大组把偶然章节当共性）。
- **卡片质量**：5 篇原"少量文本层"论文（2019 A/B/C、2020 A/B 编号论文）补 OCR 后重提炼，
  全部补齐 highlights/quotes/figures，无 fallback 卡片残留；全部 quotes ≤ 80 字。
- **防过拟合**：注入 Top-K 按题号（YYYY_T）去重，避免同一道题多篇获奖论文风格扎堆；
  排序加入 quality_score 权重；相关性阈值语义调整为"题型命中即注入，字面重合用于排序"。
- **评估**：留一验证覆盖 93 张卡片；三道 real_tests 真实赛题检索均命中正确题型指南
  （NIPT→evaluation、碳化硅→physics、绿色物流→optimization）。
