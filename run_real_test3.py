"""端到端运行 real_test3（2025 国赛 B 题：碳化硅外延层厚度），启用详细日志并捕获所有产物。

用法：
    python run_real_test3.py
"""
...
logger = logging.getLogger("e2e_test3")


def main() -> None:
    problem_text = Path("real_test3/problem.txt").read_text(encoding="utf-8")
    ...
    # real_test3 有 4 个附件（碳化硅附件1/2 + 硅附件3/4）
    attachment_paths = [
        str(Path(f"real_test3/附件/附件{i}.xlsx").resolve())
        for i in range(1, 5)
    ]
    ...
    config = {"configurable": {"thread_id": "e2e-real-test3"}}
