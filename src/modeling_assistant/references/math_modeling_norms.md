...
针对「选什么 / 怎么分 / 怎么分段」型问题（最优时点、分组策略、风险分层、分段拟合、
区间切分等），建模必须显式给出「四件套」，缺一不可：

### 目标函数

- 明确优化方向（最大化/最小化）与量纲；分组问题常用「组内一致性 + 组间区分度」
  的加权组合；分段问题常用拟合误差（如分段线性/样条的 SSE）加分段数惩罚。
- 每个目标必须能由决策变量显式写出，禁止只写「提高质量」这类不可计算的目标。

### 决策变量

- 列出全部决策变量及类型（连续/整数/0-1 二值）与上下界；
  分组问题通常是 0-1 归属变量或分组边界（断点）；分段问题是断点位置与各段参数。
- 决策变量必须与结果契约（result_contract）一一对应，
  避免「模型算出来了但结果文件里没有这个变量」。

### 约束

- 写全可行域约束：每个样本必须且只能归入一组（完备性 + 互斥）、组非空、
  断点单调递增、分段覆盖全定义域、段间边界条件衔接（连续/可导视问题要求）。
- 先写可行性约束再写目标函数：不可行模型比目标差的模型更致命。

### 分组/分段质量度量

- 分组质量：组内方差/离散度、组间均值差或分离度、每组样本量均衡度、
  业务可解释性（如孕周分组要有临床意义），并给出指标计算公式。
- 分段质量：各段拟合误差、段数-误差权衡（肘部法则/信息准则）、段间跳跃量。
- 度量必须可计算、可复现，最终以表格/图呈现各分组/分段的数值结果。
