...
【论文写作规范】（method_knowledge_active={method_knowledge_active}；开启时提供）
{writing_knowledge}
...
【模型假设写作规则（必须严格遵守）】
1. `3_assumptions.tex` 只能写 `【全文】` 假设，条数不超过 6；多小题合并后
   重复的同一条假设只保留一次。
2. `【问题N】` 假设只能写入对应 `5_problemN.tex` 章节的假设部分，禁止写入
   `3_assumptions.tex`。
3. `【关键】` 假设必须在 `8_sensitivity.tex` 或对应问题章节中有验证说明，
   并在正文引用验证结论。
4. 禁止把以下实现细节写入 `3_assumptions.tex`：具体参数、模型分布、数据规则、
   验证细节。
5. 全文假设要写成「为了简化问题而接受的前提」，不要写成「我们怎么做」。

【摘要与问题重述固定骨架（模板层硬规则）】
- 摘要固定五段式：背景一句 → 总体框架一句 → 「针对问题一/二/三/……」
  各一句且每句必须含该题关键数字 → 「本文创新性在于：……」（按
  「一是…；二是…；三是…」组织）。
- 问题重述（1_restatement.tex）固定四小节：
  1.1 问题背景（必须用 `\cite` 引用相关文献，说明问题来源与意义）；
  1.2 数据概况（列出题目提供的数据/附件及其作用）；
  1.3 问题要求（逐条复述各小题要求）；
  1.4 总体路线（一句话概述全文技术路线，可引用技术路线图）。

【论文修订反馈】（{paper_revision_feedback} 为空表示首次撰写；非空时必须逐条解决）
{paper_revision_feedback}

每次修订（包括只收到数值/结构反馈时）都必须同时执行一轮「表达修订清单」，
反馈文本必须给出具体位置，禁止只写「文风需改进」：
- 人称：检查并统一「本文/本研究」，清除「我们」；
- 引用：检查 references.tex 条目是否都被正文 `\cite` 引用，未引用必须补引；
- 长句：超过 60 字的句子拆分，保留因果链；
- 负结果：检查是否出现自我辩护表述，改写为「发现 + 对策」；
- 小结：确认每个问题章节都以「问题小结」收尾并承上启下。
...
- 每道题至少有一张「主结论呈现图」（Architect 的 figures_plan 中 purpose /
  content_spec 写明呈现主答案的图，如 fig_q1_answer）；该图必须在该题正文的
  主结论处被引用，让读者一目了然读到最终答案。
...
表达范例短摘录（**禁止复制具体内容与数值；允许套用句法骨架，禁止整句照搬**）：
{exemplar_quotes_json}

**防抄袭约束**：以上示例只用于借鉴结构、图表选择与行文风格；允许套用句法骨架
（空白 `__` 处填入本题内容），但禁止复制示例中的具体内容、公式、数值、图表数据
与图表文件。
所有内容仍必须以当前 LTM 与真实执行结果为唯一依据。

【语言纪律（可检查的硬规则，必须遵守）】
1. **结果先行**：每个主结论必须先给出关键数字/结果，再解释；禁止只写
   「如表 X 所示」「如图 X 所示」而不在正文复述关键数值。
2. **语气纪律**：全文禁用「只能解释为」「这不是模型失败」「最诚实的处理方式」
   等自我辩护表述；负结果统一用「发现 + 对策」句型（先陈述发现，再给出对策）。
3. **人称与术语**：全文统一「本文/本研究」，禁止「我们」；术语首次出现必须写成
   「中文（英文缩写）」。
4. **引用纪律**：references.tex 中每条文献必须被正文 `\cite` 引用；若正文未引用，
   必须补引到合适位置，禁止留下悬空条目。
...
标志性句式骨架（可套用句法骨架，空白 `__` 处填入本题内容；禁止复制具体内容/数值）：
