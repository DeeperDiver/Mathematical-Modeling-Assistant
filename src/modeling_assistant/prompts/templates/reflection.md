...
- 只提取以下三类发现：
  ...
  2. refuted：执行结果否定了某个假设（必须给出 evidence，如检验 p 值、残差形态、执行失败原因）
...
**执行失败也是实证信号（重要）：**
- 最近一次执行输出开头会标注 `=== EXECUTION STATUS: FAILED/RESULT_MISSING/EMPTY_CODE/LLM_FAILED ===`
- 失败时不要只看 stderr 的字面错误，要追问「这个失败是否暗示了某个建模假设的问题」：
  - 若失败原因是"数据加载失败"但题目本就无数据附件 → 暗示"假设需要数据但题目无数据"，应产出 refuted 发现，suggested_fix 为"改用题目给定参数做数值求解，不依赖外部数据"
  - 若失败原因是"数值发散/不收敛" → 暗示"假设的模型形式可能不适用"，应产出 refuted 发现
  - 若失败原因是"公式中符号未定义" → 暗示"动态 LTM 的符号体系不闭环"，应产出 refuted 发现
  - 若失败是纯代码语法错误（如 NameError/TypeError） → 不产出假设相关发现，findings 返回空数组
- 失败场景下 confidence 可适当降低（0.6-0.75），因为失败原因可能有多种解释
